...
Json::Value@ CallMapMonitorApiPath(Net::HttpRequest@ req) {
    log_trace("[CallMapMonitorApiPath] Requesting: " + req.Url);
    req.Headers['Authorization'] = 'openplanet ' + CurrentOpenplanetToken();
    req.Start();
    while(!req.Finished()) { yield(); }
    return Json::Parse(req.String());
}
